...
def get_activities_by_semester(research_hotbed_id, semester):
    """Obtiene actividades filtradas por semestre con datos completos"""
    try:
...
def generate_pdf_report(research_hotbed, members, activities, semester):
    """Genera el PDF usando ReportLab con tablas responsivas"""
    buffer = BytesIO()
    ...
    # Información del semillero - Tabla responsiva
    info_data = [
    ...
    # Resumen estadístico - Tabla más compacta
    ...
    stats_table.setStyle(TableStyle([
        ...
        ('TEXTCOLOR', (1, 0), (1, -1), colors.HexColor('#db2777')),
        ('TEXTCOLOR', (3, 0), (3, -1), colors.HexColor('#db2777')),
    ...
    # Tabla de miembros - Más compacta
    story.append(Paragraph("MIEMBROS DEL SEMILLERO", heading_style))
    
    if active_members:
        members_data = [['Nombre', 'ID SIGAA', 'Tipo', 'Ingreso']]
        for member in active_members:
            ...
            members_data.append([
                name,
                member['idSigaa'],
                member['type'][:10] + '...' if len(member['type']) > 10 else member['type'],
                member['dateEnter'].strftime('%m/%Y') if member['dateEnter'] else 'N/A'
            ])
    ...
    # Actividades detalladas - Nuevo formato
    ...
    if activities:
        ...
        for activity_type in activity_types:
            ...
            for i, activity in enumerate(type_activities, 1):
                ...
                basic_info = [
                    ['Título:', activity['title'][:60] + '...' if len(activity['title']) > 60 else activity['title']],
                ...
                if 'project_data' in activity:
                    ...
                    basic_info.extend([
                        ['Nombre proyecto:', project['name'][:50] + '...' if len(project['name']) > 50 else project['name']],
                        ['Ref. número:', project['reference_number']],
                        ['Investigador principal:', project['principal_researcher']],
                        ['Fechas:', f"{project['start_date'].strftime('%m/%Y') if project['start_date'] else 'N/A'} - {project['end_date'].strftime('%m/%Y') if project['end_date'] else 'N/A'}"]
                    ])
                    ...
                    basic_info.extend([
                        ...
                        ['Descripción:', product['description'][:80] + '...' if len(product['description']) > 80 else product['description']]
                    ])
                
                elif 'recognition_data' in activity:
                    recognition = activity['recognition_data']
                    basic_info.extend([
                        ['Proyecto:', recognition['project_name'][:50] + '...' if len(recognition['project_name']) > 50 else recognition['project_name']],
                        ['Organización:', recognition['organization_name'][:50] + '...' if len(recognition['organization_name']) > 50 else recognition['organization_name']]
                    ])
                
                # Autores
                if activity['authors']['main_authors']:
                    authors_text = ', '.join(activity['authors']['main_authors'])
                    if len(authors_text) > 60:
                        authors_text = authors_text[:60] + '...'
                ...
                if activity['authors']['co_authors']:
                    ...
                    if len(co_authors_text) > 60:
                        co_authors_text = co_authors_text[:60] + '...'
                ...
                if activity['description'] and len(activity['description']) > 100:
                    story.append(Spacer(1, 5))
                    desc_style = ParagraphStyle('Description', parent=styles['Normal'], fontSize=8, leading=10)
                    description = Paragraph(f"<b>Descripción completa:</b> {activity['description']}", desc_style)
